    ...
    tryCatch({
      ...
      for(game in 1:NumGames){
        ...
        tryCatch({
            ...
            rvest::follow_link(Session, "Final")
            
            AwayTeam_FGM <- ReadPage %>% rvest::html_nodes("#box-score-basic-coppin-state .thead td:nth-child(3)") %>% html_text()
            
        tmp_df <- data.frame(VARIABLES)
